    ...
    class CardKyoto
{
    ...
    public static function Kyoto_30($player_id, $day, $force=0)
    {
        ...
        if((($ret[2] >= 2)&&($ret[0] >= 2))||($force ==1))
        {
            ...
            {
                $d = $i+1;
                $tokyo = count(Table::getObjectListFromDB( "SELECT card_id id FROM tokyo WHERE  card_location_arg = {$player_id} AND finallocation = 2 AND card_location LIKE 'cardposition_{$d}%'", true ));
                $kyoto = count(Table::getObjectListFromDB( "SELECT card_id id FROM kyoto WHERE  card_location_arg = {$player_id} AND finallocation = 2 AND card_location LIKE 'cardposition_{$d}%'", true ));
                $count = $tokyo + $kyoto;
    ...
    public static function Kyoto_72($player_id, $day, $force=0)
    {
        ...
        if(($ret[4] >= 2)||($force ==1))
        {
            $score = 5;
            Table::DbQuery( "UPDATE player set {$day} = {$day} + {$score}  WHERE player_id = {$player_id}" );
            Table::DbQuery( "UPDATE player set wild = wild +2  WHERE player_id = {$player_id}" );
            letsgotojapan::$instance->MajPannel($player_id);
